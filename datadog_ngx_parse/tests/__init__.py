...
class TestNGiNXParse(unittest.TestCase):
    def test_should_find_remote_addr(self):
        parsed_log = parse(LOG_FORMAT_TIME_COMBINED)
        self.assertEqual(parsed_log['remote_addr'], '1.1.1.1')
        self.assertEqual(parsed_log['status'], '200')
        self.assertTrue(all(k in parsed_log for k in ('time_local', 'remote_addr')))

    def test_normalize(self):
        parsed_log = parse(LOG_FORMAT_TIME_COMBINED)
        normalized = normalize(parsed_log)
